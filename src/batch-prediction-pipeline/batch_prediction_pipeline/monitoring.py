...
def compute(feature_view_version: Optional[int] = None) -> None:
    ...
    predictions = utils.read_object_from(object_name=f"predictions_monitoring.parquet")
    ...
    utils.write_object_to(
        object_name=f"metrics_monitoring.parquet",
        data=metrics,
    )
    latest_observations = latest_observations.rename(
    ...
    utils.write_object_to(
        object_name=f"y_monitoring.parquet",
