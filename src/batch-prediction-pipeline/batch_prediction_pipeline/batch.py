...
def save(X: pd.DataFrame, y: pd.DataFrame, predictions: pd.DataFrame):
    ...
    for df, object_name in zip(
        [X, y, predictions], ["X.parquet", "y.parquet", "predictions.parquet"]
    ):
        logger.info(f"Saving {object_name} to bucket...")
        utils.write_object_to(
            object_name=object_name,
            data=df,
        )
        logger.info(f"Successfully saved {object_name} to bucket.")
...
def save_for_monitoring(predictions: pd.DataFrame, start_datetime: datetime):
    ...
    cached_predictions = utils.read_object_from(
        object_name=f"predictions_monitoring.parquet"
    )
